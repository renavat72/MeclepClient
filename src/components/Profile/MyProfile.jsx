...
export default function MyProfileWindow(props) {
  ...
  return (
    <Dialog open={isOpen} onClose={() => handleOpen()} maxWidth="xl">
      {dialogWindow === 0 ? (
        <FollowersDialog userInfo={data && data.getAuthUser.followers} />
      ) : dialogWindow === 1 ? (
        <FollowingDialog userInfo={data && data.getAuthUser.following} />
      ) : null}
      <Flex minWidth={['500px', '700px']} minHeight="470px" flexDirection="column" zIndex={200}>
        <BackgroundImage
          minWidth={['500px', '700px']}
          imageBackground={data && data.getAuthUser.coverImage}
        />
        <Flex ml="auto">
        ...
        <Flex flexDirection="column" mx="auto" width="300px" sx={{ zIndex: 100 }}>
          ...
          <Flex flexDirection="row" mx="auto" mb={3} justifyContent="center" width={1}>
            <Text fontSize={24} mr={2} textAlign="center" color="white">
              {data && data.getAuthUser.firstName}
            </Text>
            <Text fontSize={24} textAlign="center" color="white">
              {data && data.getAuthUser.secondName}
            </Text>
          </Flex>
            ...
            <Box>
              ...
              <Button size="small" onClick={() => setDialogWindow(0, !isOpen)}>
                <Text textAlign="center" color="white">
                  {data && data.getAuthUser.followers.length}
                </Text>
              </Button>
            </Box>
            <Box mx={3}>
        ...
        <Flex flexDirection="column" m={3}>
          <Box my={5}>
            <Flex flexDirection="row">
              ...
              {data &&
                data.getAuthUser.images.map((image) => (
