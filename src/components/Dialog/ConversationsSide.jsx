...
import {GET_CONVERSATIONS} from '../../apis/MessageAPI'
...
export default function ConversationsSide(props){
    ...
    return(
        <FriendsSide variables={variables}width={2/6}>
        <Box mt={2} width={1}>
            {data && data.getConversations.map((user) =>
                <FriendsBlock mx={3} flexDirection="row" p={2} mb={1} key={user.id} onClick={(()=>setFriendInfo(user.id))}>
